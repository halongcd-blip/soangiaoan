...
# -----------------------------------------------------------------
# 2. HÀM XỬ LÝ WORD (ĐÃ SỬA ĐỂ TỰ ĐỘNG XÓA LỖI TRONG WORD)
# -----------------------------------------------------------------

def create_word_document(markdown_text, ten_bai):
    
    # --- LOGIC LÀM SẠCH CHUỖI ĐẦU VÀO (CHỈ XÓA PHẦN MÃ NGUỒN VÀ TIÊU ĐỀ GRAPHVIZ) ---
    
    # DÒNG ĐÃ THÊM 1: Loại bỏ toàn bộ khối mã Graphviz thô (từ [START_GRAPHVIZ] đến [END_GRAPHVIZ])
    markdown_text = re.sub(r'\[START_GRAPHVIZ\].*?\[END_GRAPHVIZ\]', '', markdown_text, flags=re.DOTALL | re.IGNORECASE)

    # DÒNG ĐÃ THÊM 2: Loại bỏ chính xác dòng tiêu đề "PHẦN VI. SƠ ĐỒ TƯ DUY (MÃ NGUỒN GRAPHVIZ)"
    cleaned_text = re.sub(r'PHẦN\s*VI\.\s*SƠ ĐỒ TƯ DUY\s*\(MÃ NGUỒN GRAPHVIZ\)', '', markdown_text, flags=re.IGNORECASE)
    
    # DÒNG ĐÃ THÊM 3: Loại bỏ thẻ <br/> (Nếu AI vẫn tạo)
    cleaned_text = re.sub(r'<\\s*br\\s*\\/?>', '\n', cleaned_text, flags=re.IGNORECASE)
    
    # ---------------------------------------------------------------
    # BẮT ĐẦU PHẦN TẠO DOCUMENT (Toàn bộ phần tạo Word sau đó giữ nguyên logic cũ của bạn)
    document = Document()
    
    # Thiết lập style, font, size, margin, ...
    style = document.styles['Normal']
    font = style.font
    font.name = 'Times New Roman'
    font.size = Pt(12)
    
    # Xử lý chuỗi (đã được làm sạch ở trên) để chuyển thành document
    for line in cleaned_text.split('\n'):
        ...
        # Xử lý tiêu đề H1 (hoặc các tiêu đề tương tự)
        if line.startswith('**I.') or line.startswith('I.') or line.startswith('**II.') or line.startswith('II.'):
        ...
        # Xử lý gạch đầu dòng Markdown
            ...
            p.style = 'List Bullet'
            p.add_run(line[2:])
        # Xử lý đoạn văn bản thường và các định dạng in đậm/nghiêng
        else:
    ...
    # Lưu document vào bộ nhớ (BytesIO)
    bio = BytesIO()
    document.save(bio)
    bio.seek(0)
    return bio.getvalue()
...
# -----------------------------------------------------------------
# 3. PHẦN MAIN CỦA STREAMLIT (GIỮ NGUYÊN, CHỈ THÊM LÀM SẠCH CHUỖI)
# -----------------------------------------------------------------

st.title("Giáo án Tự động (AI)")
ten_bai = st.text_input("Nhập Tên Bài Học", value="") # Giả định có input này
...
if ten_bai and st.button("Tạo Giáo án"):
    ...
    if full_text:
        try:
            # *******************************************************************
            # BƯỚC SỬA LỖI TỐI THIỂU: Xóa các phần thừa trong chuỗi full_text
            # *******************************************************************
            
            # 1. Xóa toàn bộ khối mã Graphviz thô (từ [START_GRAPHVIZ] đến [END_GRAPHVIZ])
            full_text = re.sub(r'\[START_GRAPHVIZ\].*?\[END_GRAPHVIZ\]', '', full_text, flags=re.DOTALL | re.IGNORECASE)

            # 2. Xóa tiêu đề MÃ NGUỒN GRAPHVIZ (nếu AI vẫn tạo)
            ...
            # Tìm vị trí bắt đầu của Giáo án (PHẦN I. YÊU CẦU CẦN ĐẠT)
            start_index = full_text.find("PHẦN I. YÊU CẦU CẦN ĐẠT")
            
            if start_index != -1:
                cleaned_text_display = full_text[start_index:]
            else:
                # Nếu không tìm thấy, cố gắng tìm biến thể cũ hơn (I. Yêu cầu cần đạt)
                start_index = full_text.find("I. Yêu cầu cần đạt")
                if start_index != -1:
                    cleaned_text_display = full_text[start_index:]
                else:
                    # Nếu vẫn không tìm thấy, hiển thị toàn bộ chuỗi đã làm sạch
                    cleaned_text_display = full_text

            st.markdown(cleaned_text_display) # Hiển thị trên Streamlit

            # BẮT ĐẦU KHỐI CODE TẢI XUỐNG WORD
            # Truyền chuỗi đã được làm sạch (full_text đã được làm sạch) vào hàm tạo Word
            word_bytes = create_word_document(full_text, ten_bai) # Lưu ý: đã đổi tên biến từ cleaned_text thành full_text trong main block để thống nhất

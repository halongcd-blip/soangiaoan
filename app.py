...
# -----------------------------------------------------------------
# **PROMPT GỐC (ĐÃ ĐƯỢC BỔ SUNG ĐẦY ĐỦ)**
# -----------------------------------------------------------------
PROMPT_GOC = """
...
# -----------------------------------------------------------------
# 2. HÀM XỬ LÝ WORD (ĐÃ CHỈNH SỬA LOGIC LÀM SẠCH CHÍNH XÁC)
# -----------------------------------------------------------------
...
def create_word_document(markdown_text, ten_bai):
    ...
    # 1. Loại bỏ toàn bộ khối mã Graphviz thô (từ [START_GRAPHVIZ] đến [END_GRAPHVIZ])
    #    Sử dụng cờ re.DOTALL để khớp với ký tự xuống dòng (\n)
    markdown_text = re.sub(r'\[START_GRAPHVIZ\].*?\[END_GRAPHVIZ\]', '', markdown_text, flags=re.DOTALL | re.IGNORECASE)

    # 2. Loại bỏ chính xác dòng tiêu đề "PHẦN VI. SƠ ĐỒ TƯ DUY (MÃ NGUỒN GRAPHVIZ)"
    #    Regex này chỉ khớp với tiêu đề MÃ NGUỒN, **KHÔNG** ảnh hưởng đến tiêu đề "GỢI Ý SƠ ĐỒ TƯ DUY"
    cleaned_text = re.sub(r'PHẦN\s*VI\.\s*SƠ ĐỒ TƯ DUY\s*\(MÃ NGUỒN GRAPHVIZ\)', '', markdown_text, flags=re.IGNORECASE)
    
    # 3. Loại bỏ thẻ <br/> (Giữ nguyên logic cũ)
    cleaned_text = re.sub(r'<\\s*br\\s*\\/?>', '\n', cleaned_text, flags=re.IGNORECASE)
    
    # *Phần PHẦN VI. GỢI Ý SƠ ĐỒ TƯ DUY VÀ NỘI DUNG CỦA NÓ ĐƯỢC GIỮ LẠI NGUYÊN VẸN*

    # ---------------------------------------------------------------
    # BẮT ĐẦU PHẦN TẠO DOCUMENT (Toàn bộ phần tạo Word sau đó giữ nguyên)
    document = Document()
    ...
    # VÍ DỤ CỦA LOGIC CHUYỂN ĐỔI MARKDOWN THÀNH DOCX (GIỮ NGUYÊN LOGIC CŨ CỦA BẠN):
...
if ten_bai and st.button("Tạo Giáo án"):
    ...
    if full_text:
        try:
            # 1. LÀM SẠCH CHUỖI GỐC CHO CẢ HIỂN THỊ VÀ WORD
            cleaned_text = full_text
            
            # Loại bỏ mã Graphviz thô
            cleaned_text = re.sub(r'\[START_GRAPHVIZ\].*?\[END_GRAPHVIZ\]', '', cleaned_text, flags=re.DOTALL | re.IGNORECASE)
            
            # Loại bỏ dòng tiêu đề MÃ NGUỒN GRAPHVIZ (Chỉ tiêu đề này)
            cleaned_text = re.sub(r'PHẦN\s*VI\.\s*SƠ ĐỒ TƯ DUY\s*\(MÃ NGUỒN GRAPHVIZ\)', '', cleaned_text, flags=re.IGNORECASE)
            
            # Loại bỏ thẻ <br/>
            cleaned_text = re.sub(r'<\\s*br\\s*\\/?>', '\n', cleaned_text, flags=re.IGNORECASE)

            # --- LÀM SẠCH CHO HIỂN THỊ (cleaned_text_display) ---
            # Cắt chuỗi từ "I. Yêu cầu cần đạt" cho phần hiển thị
            start_index = cleaned_text.find("I. Yêu cầu cần đạt")
            
            if start_index != -1:
                cleaned_text_display = cleaned_text[start_index:]
            else:
            ...
            # Truyền chuỗi đã được làm sạch (cleaned_text) vào hàm tạo Word
